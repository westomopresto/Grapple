...
{
	SetKeyboardFocus();
	UE_LOG(LogAutoSettings, Verbose, TEXT("BindCapturePrompt: SetKeyboardFocus"));

	SetUserFocus(GetOwningPlayer());
	UE_LOG(LogAutoSettings, Verbose, TEXT("BindCapturePrompt: SetUserFocus to %s"), *GetOwningPlayer()->GetHumanReadableName());

	UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Listening for input"));
}
...
FReply UBindCapturePrompt::NativeOnMouseButtonDown(const FGeometry & InGeometry, const FPointerEvent & InMouseEvent)
{
	...
	{
		UE_LOG(LogAutoSettings, Verbose, TEXT("BindCapturePrompt: NativeOnMouseButtonDown"));
...
FReply UBindCapturePrompt::NativeOnMouseButtonUp(const FGeometry & InGeometry, const FPointerEvent & InMouseEvent)
{
	...
	if (!ShouldIgnoreEvent(InMouseEvent))
	{
		UE_LOG(LogAutoSettings, Verbose, TEXT("BindCapturePrompt: NativeOnMouseButtonUp"));
		if (CaptureMode != EBindingCaptureMode::OnReleased)
		{
			return FReply::Handled();
		}
...
void UBindCapturePrompt::Capture(FKey PrimaryKey)
{
	...
	{
		UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Escape key pressed: %s - Cancelling"), *PrimaryKey.ToString());
	...
	if (!GetDefault<UAutoSettingsConfig>()->IsKeyAllowed(PrimaryKey))
	{
		...
		UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Ignored globally disallowed key: %s"), *PrimaryKey.ToString());
		RejectCapture(Chord);
		return;
	}
	...
	{
		// Primary key disallowed, abort
		UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Ignored key disallowed by IsKeyAllowed override: %s"), *PrimaryKey.ToString());
		RejectCapture(Chord);
		return;
	}
	...
	{
		UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Rejecting key because not allowed by key grouop: %s"), *PrimaryKey.ToString());
		RejectCapture(Chord);
		return;
	}

	UE_LOG(LogAutoSettings, Log, TEXT("BindCapturePrompt: Captured chord: %s"), *Chord.GetInputText().ToString());
